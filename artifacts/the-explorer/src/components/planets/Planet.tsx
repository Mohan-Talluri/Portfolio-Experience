...
void main(){
  ...
  vec3 s=mix(c1,c2,b1);s=mix(s,c3,b2*0.7);s=mix(s,c4,b3*0.45);s+=vec3(0.10,0.06,0.22)*fine*0.4;
...
void main(){
  ...
  vec3 lit=light(s,vN,vP,vec3(1.0,0.35,0.08),0.92,0.08);lit+=lavaCol*lava*(0.8+night*1.4);
...
void main(){
  ...
  float depth=fbm(p*2.0+time*0.018);float latAbs=abs(normalize(p).y);
  ...
  s=light(s,vN,vP,vec3(0.25,0.85,1.0),0.10,1.1);gl_FragColor=vec4(s,1.0);
}`,
  projects: `${NOISE}${LIGHT}
uniform float time; varying vec3 vN,vP; varying vec2 vUv;
void main(){
  vec3 p=vP;float base=fbm(p*2.0+time*0.038);float swirl=fbm4(p*3.5-time*0.055);
  ...
  lit+=veinCol*vein*night*1.4;lit+=bio2*eyes*night*1.0;gl_FragColor=vec4(lit,1.0);
}`,
  timeline: `${NOISE}${LIGHT}
uniform float time; varying vec3 vN,vP; varying vec2 vUv;
void main(){
  vec3 p=vP;float terrain=fbm(p*1.8+time*0.008);float detail=fbm4(p*4.5+time*0.005);
  float crater=smoothstep(0.68,0.74,vnoise(p*3.2));float latAbs=abs(normalize(p).y);
  ...
  lit+=aur1*a1*0.65+aur2*a2*0.50;lit+=(aur1+aur2)*sss*0.35;gl_FragColor=vec4(lit,1.0);
}`,
  dreams: `${NOISE}${LIGHT}
uniform float time; varying vec3 vN,vP; varying vec2 vUv;
void main(){
  vec3 p=vP;float turb=fbm(p*1.4+time*0.048);
  ...
  vec3 L=normalize(vec3(2.0,1.5,1.0));float night=1.0-max(dot(vN,L),0.0);
  vec3 lit=light(s,vN,vP,vec3(0.72,0.25,1.0),0.38,0.42);lit+=bv*bolt*night*0.75;
  gl_FragColor=vec4(lit,1.0);
}`,
  contact: `${NOISE}
uniform float time; varying vec3 vN,vP; varying vec2 vUv;
void main(){
  vec3 p=vP;float gran=fbm4(p*3.8-time*0.14);float gran2=fbm4(p*8.0+time*0.10);
...
export default function Planet({ position, config, active }: PlanetProps) {
  ...
  // ── Attach window-level pointer listeners so drag never loses tracking ───────
  useEffect(() => {
    function onMove(e: PointerEvent) {
      if (!drag.current.active) return;
      const dx = e.clientX - drag.current.x;
      const dy = e.clientY - drag.current.y;
      // sensitivity: touch slightly higher
      const sens = e.pointerType === 'touch' ? 0.011 : 0.008;
  ...
  useFrame((state, delta) => {
    ...
    if (group) {
      if (drag.current.active) {
        // Apply velocity directly while dragging for instant response
        group.rotation.y += rotVel.current.y;
        group.rotation.x += rotVel.current.x;
      } else {
        // Inertia: decay and coast after release
        rotVel.current.x *= 0.92;
        rotVel.current.y *= 0.92;
        group.rotation.y += rotVel.current.y;
        group.rotation.x += rotVel.current.x;

        // Auto-rotate only when fully stopped
  ...
  function handlePointerDown(e: any) {
    ...
    // Kill inertia so drag starts fresh
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      {/* Orbit rings — children of group so they rotate with the planet */}
